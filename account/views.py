    ...
    def get(self, request):
        ...
        try:
            ...
            if not user.isverified:
                ...
                user.save()
            return redirect("https://ephemeral-licorice-0a30d4.netlify.app/")
...
class LoginAPIView(generics.GenericAPIView):
    ...
    def post(self, request):
        ...
        user = auth.authenticate(request,mobile_number=mobile_number, password=password)
        # print("user",user,mobile_number,serializer)
        # login(request,user)
...
class RequestPasswordRestEmail(generics.GenericAPIView):
    ...
    def post(self,request):
        ...
        if User.objects.filter(email=email).exists():
            ...
            Util.send_email(data)
        return Response({'sucess':'We have sent you a link of reset password','uidb64':uidb64,'token':token},status=status.HTTP_200_OK)
